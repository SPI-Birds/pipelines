...
request_data <- subset_datarqst(
  file = pipeline_output, PopID = request_PopIDs, Species = request_Species,
  save = FALSE
)
...
test_that("Conflicted species individuals are only included when prompted...", {
  request_data_C <- subset_datarqst(
    file = pipeline_output, PopID = request_PopIDs, Species = request_Species,
...
test_that("Conflicted species individuals return all capture records (even non-focal species)...", {
  request_data_C <- subset_datarqst(
    file = pipeline_output, PopID = "HAR", Species = "PARMAJ",
    include_conflicting = TRUE, save = FALSE
  )
  ...
  request_data_C2 <- subset_datarqst(
    file = pipeline_output, PopSpec = "HAR_PARMAJ",
    include_conflicting = TRUE, save = FALSE
  )
