context("Run data quality check on Kevo pipeline output")
...
test_that("Capture data returns an expected outcome...", {
  ...
  #Will be NA because there is no capture data, but breeding season should be correct
  expect_equal(subset(KEV_data, IndvID == "V-815188")$CaptureDate[1], as.Date(NA))
...
test_that("Location_data returns an expected outcome...", {
  ...
  expect_equal(round(subset(KEV_data, LocationID == "0001")$Latitude, 2), 67.95)
  expect_equal(round(subset(KEV_data, LocationID == "0001")$Longitude, 2), 5.61)
