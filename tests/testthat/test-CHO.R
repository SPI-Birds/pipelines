...
test_that("ID columns match the expected format for the pipeline", {
  ...
  test_ID_format(pipeline_output, column = "FemaleID", format = "^[C][:digit:]{6}$")

  ## MaleID format is as expected
  test_ID_format(pipeline_output, column = "MaleID", format = "^[C][:digit:]{6}$")

  ## IndvID format in Capture data  is as expected
  test_ID_format(pipeline_output, column = "IndvID", table = "Capture", format = "^[C][:digit:]{6}$")

  ## IndvID format in Individual data is as expected
  test_ID_format(pipeline_output, column = "IndvID", table = "Individual", format = "^[C][:digit:]{6}$")
