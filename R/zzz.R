...
\\item The second report (i.e., 'Warnings and Verified Records') contains a list of warnings, which are values that are considered possible but unlikely (e.g., a value for clutch size that is twice as large as a mean clutch size for a species). Similar to the potential errors, whenever a record is flagged as a warning, a line is added to this report with information on the type of check that was violated and the row number of the corresponding record. Again, this row number refers to the column Row in the corresponding data table in the standard format and does not refer to the row number in the primary data. Besides a list of warnings, this report contains a list of verified records. Some of the records flagged as a warning or potential error are likely to be uncommon but true observations. We don't want these same values to be flagged each time a new quality check is conducted. To overcome this, we have implemented an 'approve-listing' procedure that will prevent true records, that have been verified by the data owner, from appearing in future quality check reports. These records are listed at the end of the second document. We hope that this will make the quality check reports more useful for data owners and users.
\\end{enumerate}
...
Brood checks:
  \\begin{itemize}
\\item \\textbf{B1}. Compare clutch size and brood size. Clutch size is expected to be larger or equal to brood size. Broods that do not meet this expectation are flagged as 'warning' (in case they were experimentally manipulated) or 'potential error' (in case they were not experimentally manipulated).
...
\\item \\textbf{B3}. Compare lay date and hatch date. Lay date is expected to be earlier than hatch date. Broods that do not meet this expectation are flagged as 'potential error'.
\\item \\textbf{B4}. Compare hatch date and fledge date. Hatch date is expected to be earlier than fledge date. Broods that do not meet this expectation are flagged as 'potential error'.
...
\\item \\textbf{B5d}. Compare lay date against reference values. Reference values are generated by population- and species-specific data if the number of observations is sufficiently large (n >= 100). Records are flagged as 'warning' if they are earlier than the 1st percentile or later than the 99th percentile. Records are flagged as 'potential error' if they are earlier than January 1st or later than December 31st of the same year.
...
\\item \\textbf{B7}. Check that brood identities (BroodID) are unique within populations. BroodIDs are not expected to be unique among populations. Records with non-unique BroodIDs are flagged as 'potential error'.
\\item \\textbf{B8}. Check that the order of clutch types per breeding female per breeding season is correct. Replacement and second clutches can occur in any order but never before first clutches. First clutches that are not listed first are flagged as 'potential error'.
\\item \\textbf{B9}. Compare species of mother and father. The species of the parents are expected to be the same in the majority of broods. Common, biologically possible multi-species broods are flagged as 'warning'. Other combinations are flagged as 'potential error'.
\\item \\textbf{B10}. Compare species of parents and the brood itself. The species of the parents and their brood are expected to be the same. Broods with a combination of species for which brood fostering is known to exist are flagged as 'warning'. Other combinations of species are flagged as 'potential error'.
\\item \\textbf{B11}. Compare species of brood and the chicks in the brood. The species of the brood and the chicks are expected to be the same. Broods with a combination of species for which brood fostering is known to exist are flagged as 'warning'. Other combinations of species are flagged as 'potential error'.
\\item \\textbf{B12}. Check that the sex of the mother is female. Broods where mothers are listed as male are flagged as 'potential error'.
\\item \\textbf{B13}. Check that the sex of the father is male. Broods where fathers are listed as female are flagged as 'potential error'.
\\end{itemize}
...
\\item \\textbf{C2}. Check that the chick age values (in number of days since hatching) are within the expected duration of the nesting period. Expected number of days are between 0 and 30. Values outside this range are flagged as 'potential error'. Impossible chick age may be caused by problems with hatch date.
...
Individual checks:
  \\begin{itemize}
\\item \\textbf{I1}. Check that individual identities (IndvID) are unique within populations. IndvIDs are not expected to be unique among populations. Records with non-unique IndvIDs are flagged as 'potential error'.
\\item \\textbf{I2}. Check that the brood identities for an individual (BroodIDLaid, BroodIDFledged) match the correct nest in Capture data. Chicks caught on a nest that are not associated with corresponding brood identities are flagged as 'potential error'.
\\item \\textbf{I3}. Check for the uncertainty in the sex of an individual. Individuals that have been recorded as both male and female in Capture data are marked as conflicted ('C') by the pipeline and flagged as 'potential error'.
\\item \\textbf{I4}. Check for the uncertainty in the species of an individual. Individuals that have been recorded as different species in Capture data are marked as conflicted ('CCCCCC') by the pipeline and flagged as 'potential error'.
\\item \\textbf{I5}. Check that individuals in Individual data also appear in Capture data. Individual data are usually a direct product of Capture data, so this should never occur. If it does occur, it is an indication of problems in the underlying pipeline. Missing individuals are flagged as 'potential error'.
\\end{itemize}
