...
extract_paradox_db <- function(path, file_name){
  ...
  if(grepl(pattern = 'windows', x = OS)){

    reticulate::use_python(python = "C:\\Users\\Liam\\AppData\\Local\\Programs\\Python\\Python37")
    #NOTE: This path is specific to Liam as a user. Should verify that this actually does anything
    # and if so, it should be generalized.
  }
