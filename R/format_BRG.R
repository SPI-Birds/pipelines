    ...
    nest_data <- readxl::read_xlsx(
        ...
        dplyr::transmute(
            ...
            Species = dplyr::case_when(
                .data$Species == "Kj\u00f8ttmeis" ~ species_codes[species_codes$speciesEURINGCode == 14640, ]$Species,
                .data$Species == "Bl\u00e5meis" ~ species_codes[species_codes$speciesEURINGCode == 14620, ]$Species,
                .data$Species == "Svarthvitfluesnapper" ~ species_codes[species_codes$speciesEURINGCode == 13490, ]$Species,
                .data$Species == "Svartmeis" ~ species_codes[species_codes$speciesEURINGCode == 14610, ]$Species
            ),
            Plot = .data$Location,
...
create_capture_BRG <- function(chick_data, adult_data, Brood_data_temp) {
    ...
    Capture_data_temp <- adult_data %>%
        ...
        dplyr::mutate(IndvID = dplyr::case_when(
            stringr::str_detect(.data$IndvID, "^[:alpha:]{2}[:digit:]{5}$") ~ .data$IndvID,
            TRUE ~ NA_character_
        )) %>%
...
create_individual_BRG <- function(Capture_data_temp) {
    ...
    Individual_data_temp <- Capture_data_temp %>%
        ...
        dplyr::ungroup() %>%
        ## Reorder columns
        dplyr::select(dplyr::any_of(names(data_templates[["v1.1.0"]]$Individual_data)), tidyselect::everything())
...
create_location_BRG <- function(nest_data) {
    ...
    Location_data_temp <- nest_data %>%
        ...
        dplyr::ungroup() %>%
        dplyr::select(dplyr::any_of(names(data_templates[["v1.1.0"]]$Location_data)), tidyselect::everything())
