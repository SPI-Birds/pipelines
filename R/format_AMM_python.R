...
format_AMM_python <- function(db = choose_directory(),
                              path = ".",
                              species = NULL,
                              pop = NULL,
                              output_type = "R") {
  ...
  Capture_data <- create_capture_AMM(
  ...
  Individual_data <- create_individual_AMM(
  ...
  Location_data <- create_location_AMM(
    Capture_data = Capture_data,
    dir = table_dir,
    protocol_version = protocol_version
  )
  ...
  if (output_type == "csv") {
    ...
    utils::write.table(
      x = protocol_version, file = paste0(path, "/protocol_version_AMM.txt"),
...
create_brood_AMM <- function(dir, species_filter) {
  ...
  Brood_data <- utils::read.csv(paste0(dir, "/", "Broods", ".csv")) %>%
    ...
    dplyr::mutate(
      ...
      BroodOther_ExperimentID = dplyr::case_when(
        .data$BroodOtherTreatment %in% c(1L, 2L, 3L, 4L, 5L) ~ list("SURVIVAL"),
        TRUE ~ list(NA_character_)
      ),
      Plot_ExperimentID = dplyr::case_when(
        .data$PlotLevelTreatment %in% c(1L, 2L) ~ list("PHENOLOGY"),
    ...
    dplyr::rowwise() %>%
    dplyr::mutate(ExperimentID = paste(
      dplyr::union(
        dplyr::union(
          .data$BroodSwap_ExperimentID,
    ...
    dplyr::select("BroodID", "PopID", "BreedingSeason",
      "Species", "Plot",
      "LocationID" = "NestBox", "FemaleID", "MaleID",
...
create_capture_AMM <- function(Brood_data, dir, species_filter) {
  ...
  Adult_capture <- Catches_table %>%
    ...
    dplyr::mutate(
      Species = dplyr::case_when(
        .data$CatchSpecies == 1L ~ !!species_codes$Species[species_codes$speciesEURINGCode == "14640"],
        .data$CatchSpecies == 2L ~ !!species_codes$Species[species_codes$speciesEURINGCode == "14620"],
        .data$CatchSpecies == 3L ~ !!species_codes$Species[species_codes$speciesEURINGCode == "14610"],
        .data$CatchSpecies == 4L ~ !!species_codes$Species[species_codes$speciesEURINGCode == "14790"]
      ),
    ...
    dplyr::mutate(dplyr::across(
      .cols = c("BodyMassField", "Tarsus", "WingP3"),
  ...
  Chick_capture <- Chick_catch_tables %>%
    ...
    dplyr::mutate(
      HatchDay = dplyr::case_when(
        (.data$HatchDay >= 500 | .data$HatchDay < 0) ~ NA_integer_,
        TRUE ~ .data$HatchDay
      ),
      # All chicks are GT
      Species = species_codes$Species[species_codes$speciesEURINGCode == 14640],
    ...
    dplyr::mutate(dplyr::across(
      ...
      ~ dplyr::na_if(as.character(.), "-99")
    )) %>% # Needed because pivot functions expect coercable classes when making value col
    dplyr::mutate(dplyr::across(
      tidyselect::contains("BodyMassTime"),
      ~ stringr::str_extract(., "[0-9]{2}:[0-9]{2}")
    )) %>%
    tidyr::pivot_longer(
      cols = "Day2BodyMass":"Day18Observer",
      names_to = "column",
      values_to = "value"
    ) %>%
    dplyr::mutate(
      Day = stringr::str_extract(.data$column, "[0-9]{1,}"),
      OriginalTarsusMethod = "Alternative",
      Age_observed = 1L,
      CaptureAlive = TRUE,
      ReleaseAlive = TRUE
...
create_individual_AMM <- function(Capture_data, Brood_data, dir, protocol_version) {
  Sex_genetic <- utils::read.csv(paste0(dir, "/", "GenotypesAllYears", ".csv")) %>%
    ...
    dplyr::group_by(.data$IndvID) %>%
    dplyr::summarise(
      length_sex = length(unique(.data$SexGenetic)),
...
create_location_AMM <- function(Capture_data, dir, protocol_version) {
  Habitat_data <- utils::read.csv(paste0(dir, "/", "HabitatDescription", ".csv")) %>%
    ...
    dplyr::summarise(
      DEC = sum(c(
        .data$Beech, .data$Larch, .data$Maple, .data$Birch,
        .data$Oak, .data$Willow, .data$Poplar, .data$Alder, .data$AshTree
      )),
      EVE = sum(c(.data$Spruce, .data$Pine))
    ) %>%
    dplyr::mutate(
      perc_dec = .data$DEC / (.data$DEC + .data$EVE),
      dominant_sp = dplyr::case_when(
        .data$perc_dec >= 0.66 ~ "DEC",
  ...
  Location_data <- utils::read.csv(paste0(dir, "/", "NestBoxes", ".csv")) %>%
    ...
    dplyr::mutate(
      ...
      EndSeason = dplyr::case_when(
        nchar(.data$NestboxID) == 4 & stringr::str_sub(.data$NestboxID, 1, 2) == "16" ~ 2016L,
        TRUE ~ 2019L
      ),
