...
#' \strong{Capture_data}: Capture records are excluded if they have missing values in key columns (IndvID, Species, or CaptureDate).
#' This removes individuals that escaped before being ringed, or records with incomplete capture information.
#' @inheritParams pipeline_params
#'
#' @return Generates either 4 .csv files or 4 data frames in the standard format and 1 character vector indicating the protocol version on which the pipeline is based. When `output_type = "csv"`, 4 .csv files corresponding to the 4 standard data tables and 1 text file indicating the protocol version on which the pipeline is based.
#' @export
...
format_FOR <- function(db = choose_directory(),
  ...
  dsn <- paste0(gsub("\\\\", "/", db), "/FOR_PrimaryData.accdb")
  ...
  table_dir <- paste0(db, "/FOR_PrimaryData_tables")
  ...
  Brood_data <- create_brood_FOR(table_dir)

  # CAPTURE DATA

  message("Compiling capture information...")

  Capture_data <- create_capture_FOR(Brood_data, table_dir)

  # INDIVIDUAL DATA

  message("Compiling individual information...")

  Individual_data <- create_individual_FOR(
  ...
  Location_data <- create_location_FOR(Capture_data, table_dir)
  ...
  AvgTarsus <- Capture_data %>%
    ...
    dplyr::group_by(.data$BroodID) %>%
    dplyr::summarise(
      AvgTarsus = mean(.data$Tarsus),
      NumberChicksTarsus = dplyr::n(),
      OriginalTarsusMethod = "Alternative"
    ) # check what is the name of the method we use
  ...
  Brood_data <- Brood_data %>%
    ...
    dplyr::select(
      "BroodID":"NumberEggs", "AvgChickMass",
...
#'
#' @param table_dir Path to directory containing exported CSV tables.
#'
#' @return A data frame.

create_brood_FOR <- function(table_dir) {
  Catches_M <- read.csv(paste0(table_dir, "/", "Catches", ".csv")) %>%
  ...
  Catches_F <- read.csv(paste0(table_dir, "/", "Catches", ".csv")) %>%
  ...
  Brood_data <- read.csv(paste0(table_dir, "/", "Broods", ".csv")) %>%
    ...
    dplyr::mutate(dplyr::across(
      .cols = c("HatchDay", "FledgeDay"),
      .fns = ~ dplyr::if_else(. >= 500, NA_integer_, .)
    )) %>%
    dplyr::mutate(
      ...
      BroodSize_max = dplyr::if_else(
        !is.na(.data$ClutchSize_observed) & !is.na(.data$UnhatchedEggs) &
          (.data$ClutchSize_observed - .data$UnhatchedEggs) < 1,
        NA_integer_,
      ...
      NumberFledged_max = dplyr::if_else(
        !is.na(.data$NumberFledged) & .data$NumberFledged < 1,
        NA_integer_,
        .data$NumberFledged
      ),
    ...
    dplyr::mutate(dplyr::across(
      c("LayDate_observed", "HatchDate_observed", "FledgeDate_observed"),
      as.Date
    ))
...
create_capture_FOR <- function(Brood_data, table_dir) {
  Catches_table <- read.csv(paste0(table_dir, "/", "Catches", ".csv"))

  Chick_catch_tables <- read.csv(paste0(table_dir, "/", "Chicks", ".csv"))

  Nestbox_capture <- read.csv(paste0(table_dir, "/", "NestBoxes", ".csv")) %>%
  ...
  Adult_capture <- Catches_table %>%
    ...
    dplyr::mutate(
      ...
      CaptureTime = dplyr::if_else(
        ...
        NA_character_,
        format(strptime(.data$CatchTime, "%Y-%m-%d %H:%M:%S"), "%H:%M")
      ),
      ...
      ReleaseAlive = dplyr::case_when(
        !is.na(.data$Adultdeath) & .data$Adultdeath == 1 & !is.na(.data$DateAdultdeath) & as.Date(.data$DateAdultdeath) == .data$CaptureDate ~ FALSE,
        .default = TRUE
      ),
      OriginalTarsusMethod = "Alternative",
    ...
    dplyr::filter(!is.na(.data$IndvID), !is.na(.data$Species), !is.na(.data$CaptureDate)) %>%
    dplyr::select(
      "IndvID",
      "Species",
      "Sex_observed",
  ...
  Chick_capture <- Chick_catch_tables %>%
    ...
    dplyr::mutate(
      EndMarch = as.Date(paste(.data$ChickYear, "03", "31", sep = "-")),
      CapturePopID = "FOR",
      ReleasePopID = "FOR",
      IndvID = as.character(.data$RingNumber),
      Species = dplyr::case_match(
    ...
    dplyr::mutate(
      ...
      CaptureTime = dplyr::if_else(
        is.na(.data$Day14BodyMassTime),
        NA_character_,
        format(strptime(.data$Day14BodyMassTime, "%Y-%m-%d %H:%M:%S"), "%H:%M")
      ),
    ) %>%
    dplyr::mutate(
      ObserverID = as.character(dplyr::na_if(.data$Day14Observer, -99L)),
  ...
  Capture_data <- dplyr::bind_rows(Adult_capture, Chick_capture) %>%
    ...
    dplyr::filter(
      !is.na(.data$IndvID), !is.na(.data$Species),
      !is.na(.data$CaptureDate), !is.na(.data$BreedingSeason),
...
create_individual_FOR <- function(Capture_data, Brood_data, table_dir) {
  Sex_calc <- Capture_data %>%
    dplyr::filter(!is.na(.data$Sex_observed)) %>%
    dplyr::group_by(.data$IndvID) %>%
    dplyr::summarise(
      length_sex = length(unique(.data$Sex_observed)),
      unique_sex = list(unique(.data$Sex_observed))
    ) %>%
    dplyr::rowwise() %>%
    dplyr::mutate(Sex_calculated = dplyr::case_when(
      .data$length_sex > 1 ~ "C",
      .default = .data$unique_sex[[1]]
    )) %>%
    dplyr::ungroup() %>%
    dplyr::select("IndvID", "Sex_calculated")

  Brood_info <- read.csv(paste0(table_dir, "/", "Chicks", ".csv")) %>%
    dplyr::select("IndvID" = "RingNumber", "BroodIDLaid" = "BroodID", "BroodIDFledged" = "BroodID") %>%
    dplyr::mutate(dplyr::across(tidyselect::everything(), as.character)) %>%
    # Keep only one record per IndvID (in case of duplicates)
    dplyr::distinct(IndvID, .keep_all = TRUE)
  ...
  Individual_data <- Capture_data %>%
    ...
    dplyr::summarise(
      ...
      RingAge = dplyr::if_else(min(.data$Age_observed) != 1L | is.na(min(.data$Age_observed)),
        "adult", "chick"
      )
    ) %>%
    ...
    dplyr::select(
      "IndvID", "Species", "PopID",
      "BroodIDLaid", "BroodIDFledged",
      "RingSeason", "RingAge",
      "Sex_calculated", "Sex_genetic"
    ) %>%
    # Ensure uniqueness (in case of any remaining duplicates)
    dplyr::distinct(IndvID, .keep_all = TRUE)
