...
format_MAL <- function(db = choose_directory(),
    ...
    rr_data <- readxl::read_xlsx(
        path = paste0(db, "/MAL_PrimaryData_1.xlsx"),
        guess_max = 5000
    ) %>%
        ...
        dplyr::transmute(
            PopID = as.character("MAL"),
            BreedingSeason = as.integer(.data$Year),

            ## TODO: Check Species codes
            Species = dplyr::case_when(
                .data$Species == "TX" ~ species_codes[species_codes$speciesEURINGCode == 14640, ]$Species[1],
                .data$Species == "BM" ~ species_codes[species_codes$speciesEURINGCode == 14620, ]$Species[1],
                .data$Species == "RS" ~ NA_character_,
                .data$Species == "NA" ~ NA_character_
            ),
    ...
    nest_data_17_19 <- readxl::read_xlsx(
        path = paste0(db, "/MAL_PrimaryData_2.xlsx"),
        sheet = 1,
        guess_max = 5000
    ) %>%
        janitor::clean_names(case = "upper_camel") %>%
        ...
        dplyr::transmute(
            ...
            BreedingSeason = dplyr::case_when(
                is.na(.data$Year) ~ as.integer(lubridate::year(.data$LayDate_observed)),
                TRUE ~ as.integer(.data$Year)
            )
        )

    ## Read in primary data from nest records (old format)
    nest_data_13_16 <- readxl::read_xlsx(
        path = paste0(db, "/MAL_PrimaryData_2.xlsx"),
        sheet = 2,
        guess_max = 5000
    ) %>%
    ...
    Brood_data <- create_brood_MAL(nest_data, rr_data, protocol_version)

    #### CAPTURE DATA
    message("Compiling capture information...")
    Capture_data <- create_capture_MAL(rr_data, protocol_version)

    #### INDIVIDUAL DATA
    message("Compiling individual information...")
    Individual_data <- create_individual_MAL(Capture_data, Brood_data, protocol_version)

    #### LOCATION DATA
    message("Compiling location information...")
    Location_data <- create_location_MAL(loc_data, protocol_version)
...
create_brood_MAL <- function(nest_data, rr_data, protocol_version) {
    ...
    rr_data_brood_ads <- rr_data %>%
        ...
        dplyr::mutate(
            FemaleID = if ("F" %in% names(.)) .data[["F"]] else NA_character_,
            MaleID   = if ("M" %in% names(.)) .data[["M"]] else NA_character_
        ) %>%
        dplyr::select(-dplyr::any_of(c("F", "M")))
    ...
    rr_data_brood_cks <- rr_data %>%
        ...
        dplyr::summarise(
            AvgChickMass = round(mean(.data$Mass, na.rm = T), 1),
    ...
    Brood_data <- nest_data %>%
        ...
        dplyr::select(dplyr::contains(names(data_templates[[paste0("v", protocol_version)]]$Brood_data))) %>%
        # Add missing columns
    ...
    purrr::map_df(data_templates[[paste0("v", protocol_version)]]$Brood_data, class) == purrr::map_df(Brood_data, class)
...
create_capture_MAL <- function(rr_data, protocol_version) {
    ...
    Capture_data <- rr_data %>%
        ...
        ## Keep only necessary columns
        dplyr::select(dplyr::contains(names(data_templates[[paste0("v", protocol_version)]]$Capture_data))) %>%
        ## Add missing columns
...
create_individual_MAL <- function(Capture_data, Brood_data, protocol_version) {
    Individual_data <- Capture_data %>%
        ...
        dplyr::ungroup() %>%
        ## Keep only necessary columns
        dplyr::select(dplyr::contains(names(data_templates[[paste0("v", protocol_version)]]$Individual_data))) %>%
        ## Add missing columns
...
create_location_MAL <- function(loc_data, protocol_version) {
    ...
    Location_data <- loc_data %>%
        ...
        dplyr::ungroup() %>%
        ## Keep only necessary columns
        dplyr::select(dplyr::contains(names(data_templates[[paste0("v", protocol_version)]]$Location_data))) %>%
        ## Add missing columns
