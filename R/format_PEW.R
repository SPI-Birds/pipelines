...
format_PEW <- function(db = choose_directory(),
  ...
  pew_data <- readxl::read_excel(path =  paste0(db, "/PEW_PrimaryData.xlsx"),
    ...
    dplyr::mutate(BreedingSeason = as.integer(.data$Year),
                  ...
                  Species = species_codes[which(species_codes$speciesCode == 10002), ]$speciesID,
                  PopID = "PEW",
...
create_brood_PEW <- function(data) {
  ...
  parents_brood_data <-
    ...
    dplyr::select(-c(.data$IndvID, .data$Sex, .data$Age, .data$PartnerId, .data$NumberTransponder,
    ...
    dplyr::left_join(parent_info, by = "BroodID") %>%
