...
format_PEW <- function(db = choose_directory(),
  ...
  pew_data <- readxl::read_excel(path =  paste0(db, "/PEW_PrimaryData.xlsx"),
    ...
    dplyr::mutate(DateEgg1 = janitor::excel_numeric_to_date(as.numeric(DateEgg1),
                                                            date_system = "modern")) %>%
    dplyr::distinct()
  ...
  Capture_data <-
    ...
    dplyr::mutate(HatchDate_observed = if_else(is.na(HatchDate_observed) & is.na(CaptureDate) & Method == "ChickRinging",
                  ...
                  CaptureDate = case_when(is.na(CaptureDate) & D14Chicks == "yes" ~ HatchDate_observed + 14,
                                          ...
                                          is.na(CaptureDate) ~ as.Date(paste0(BreedingSeason, "-04-01"), "%Y-%m-%d"),
                                          TRUE ~ CaptureDate)) %>%
    ...
    dplyr::arrange(Year, CaptureDate, .by_group = TRUE) %>%
...
create_location_PEW <- function(data) {
  ...
  Location_data <-
    ...
    group_by(NestboxID) %>%
    arrange(BreedingSeason, CaptureDate, .by_group = TRUE) %>%
