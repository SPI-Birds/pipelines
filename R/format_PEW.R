...
create_brood_PEW <- function(data) {
  ...
  parents_brood_data <-
    ...
    filter(.data$Method %in% c("Catch adults nestbox", "Catch incubation")) %>%
    #### Rename variables
...
create_capture_PEW <- function(pew_data, Brood_data) {
  ...
  Capture_data <-
    ...
    dplyr::mutate(HatchDate_observed = if_else(is.na(.data$HatchDate_observed) & is.na(.data$CaptureDate) &
    ...
    dplyr::mutate(ChickAge = if_else(.data$Age_observed == 1L,
...
create_location_PEW <- function(data) {
  ...
  Location_data <-
    ...
    arrange(.data$BreedingSeason, .data$CaptureDate, .by_group = TRUE) %>%
