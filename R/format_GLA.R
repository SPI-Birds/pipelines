...
format_GLA <- function(db = choose_directory(),
  ...
  nest_data <- readxl::read_xlsx(
    path = paste0(db, "/GLA_PrimaryData_Nest.xlsx"),
    guess_max = 5000
  ) %>%
    ...
    dplyr::mutate(
      BreedingSeason = as.integer(.data$Yr),
    ...
    dplyr::mutate(dplyr::across(
      .cols = c("FirstEggDate", "LayingComplete", "ObservedHatch"),
      .fns = ~ suppressWarnings(dplyr::case_when(
        grepl("/", .) ~ lubridate::dmy(., quiet = TRUE),
        TRUE ~ lubridate::ymd(
          paste(
            unlist(stringr::str_split(as.character(janitor::excel_numeric_to_date(as.numeric(.))),
              pattern = "-"
            ))[c(1, 3, 2)],
            collapse = "-"
          ),
    ...
    dplyr::mutate(
      ClutchSize_max = dplyr::case_when(
        .data$ClutchComplete == "TRUE" ~ as.numeric(.data$ClutchSize),
        .data$ClutchComplete == "FALSE" ~ Inf
      ),
      Species = dplyr::case_when(
        .data$Species == "greti" ~ species_codes[species_codes$speciesEURINGCode == 14640, ]$Species,
      ...
      BroodSize_observed = .data$Hatchlings,
      PopID = dplyr::case_when(
        .data$PopID == "cashel" ~ "CAS",
  ...
  rr_data <- readxl::read_xlsx(
    path = paste0(db, "/GLA_PrimaryData_RingingRecords.xlsx"),
    col_types = c(
      rep("text", 7),
      "date", "date",
      rep("text", 31)
    ...
    dplyr::mutate(
      ...
      CaptureTime = dplyr::case_when(
        format(.data$CaptureTime, format = "%H:%M:%S") == "00:00:00" ~ NA_character_,
      ...
      Sex_observed = dplyr::case_when(
        grepl(pattern = "F", .data$Sex_observed) ~ "F",
    ...
    dplyr::mutate(PopID = dplyr::case_when(
      .data$PopID == "cashel" ~ "CAS",
      .data$PopID == "garscube" ~ "GAR",
      .data$PopID == "kelvingrove_park" ~ "KEL",
      .data$PopID == "sallochy" ~ "SAL",
  ...
  if (output_type == "csv") {
    ...
    utils::write.table(
      x = protocol_version, file = paste0(path, "\\protocol_version_GLA.txt"),
...
create_brood_GLA <- function(nest_data, rr_data, protocol_version) {
  ...
  rr_data_brood_sum <- rr_data %>%
    ...
    dplyr::reframe(
      FemaleID = dplyr::case_when(
        dplyr::n_distinct(.data$MotherRing) > 1 ~ NA_character_,
        TRUE ~ unique(.data$MotherRing)
      ),
      MaleID = dplyr::case_when(
        dplyr::n_distinct(.data$FatherRing) > 1 ~ NA_character_,
  ...
  nest_data_brood_sum <- nest_data %>%
    dplyr::mutate(ClutchType_observed = dplyr::case_when(
      is.na(.data$ReplacementClutch) ~ NA_character_,
  ...
  Brood_data <- nest_data_brood_sum %>%
    dplyr::group_by(.data$BreedingSeason, .data$PopID, .data$Species, .data$NestboxNumber) %>%
    dplyr::mutate(last_rec = dplyr::case_when(
      .data$BroodID == max(.data$BroodID) ~ "yes",
      TRUE ~ "no"
    )) %>%
    ...
    dplyr::left_join(
      rr_data_brood_sum %>%
        dplyr::mutate(chicks_fledged = dplyr::case_when(
          !is.na(.data$NumberChicksMass) | !is.na(.data$NumberChicksTarsus) ~ "yes",
          TRUE ~ "no"
        )),
    ...
    dplyr::mutate(
      MaleID_j = dplyr::case_when(
        .data$MaleID.x == .data$MaleID.y ~ .data$MaleID.x,
        is.na(.data$MaleID.x) & !is.na(.data$MaleID.y) ~ .data$MaleID.y,
        !is.na(.data$MaleID.x) & is.na(.data$MaleID.y) ~ .data$MaleID.x
      ),
      FemaleID_j = dplyr::case_when(
        .data$FemaleID.x == .data$FemaleID.y ~ .data$FemaleID.x,
    ...
    dplyr::mutate(Species_j = dplyr::case_when(
      .data$Species.x == .data$Species.y ~ .data$Species.x,
    ...
    dplyr::mutate(dplyr::across(
      ...
      .fns = ~ dplyr::case_when(
        stringr::str_detect(., "^[[:digit:][:alpha:]]{7}$") ~ .,
        TRUE ~ NA_character_
      )
    )) %>%
    ...
    dplyr::mutate(LocationID = dplyr::case_when(
      is.na(.data$NestboxNumber) ~ NA_character_,
...
create_capture_GLA <- function(nest_data, rr_data, Brood_data, protocol_version) {
  ...
  Capture_data_rr <- rr_data %>%
    ...
    dplyr::mutate(
      ...
      CaptureAlive = dplyr::case_when(
        .data$Retrap == "X" ~ FALSE,
        .data$Retrap %in% c("N", "R", "C", "U", NA) ~ TRUE
      ), ## Set CaptureAlive to FALSE if Retrap is X, otherwise TRUE
      ReleaseAlive = dplyr::case_when(
        .data$Retrap == "X" | .data$Age == "X" ~ FALSE,
        .data$Retrap %in% c("N", "R", "C", "U", NA) ~ TRUE
      ), ## Set ReleaseAlive to FALSE if Retrap is X or if Age is X (chick found dead at nest)
      ReleasePopID = dplyr::case_when(
        .data$ReleaseAlive == FALSE ~ NA_character_,
  ...
  Capture_data_nest <-
    ...
    dplyr::mutate(
      Sex_observed = dplyr::case_when(
        grepl("Female", .data$Sex_observed) ~ "F",
        grepl("Male", .data$Sex_observed) ~ "M"
      ),
      CapturePopID = .data$PopID,
      CaptureDate = dplyr::case_when(
        is.na(.data$LayDate_observed) ~ as.Date(paste0(.data$BreedingSeason, "-05-15")),
  ...
  Capture_data <- Capture_data_rr %>%
    ...
    dplyr::mutate(IndvID = dplyr::case_when(
      stringr::str_detect(.data$IndvID, "^[[:digit:][:alpha:]]{7}$") ~ .data$IndvID,
      TRUE ~ NA_character_
    )) %>%
    dplyr::filter(!is.na(.data$IndvID)) %>%
    # Add PopID prefix to LocationID to avoid duplicated LocationIDs across sites
...
create_individual_GLA <- function(Capture_data, Brood_data, protocol_version) {
  ...
  Individual_data <- Individual_data_temp %>%
    ...
    dplyr::group_by(.data$IndvID) %>%
    dplyr::mutate(
      BroodIDFledged = purrr::map_chr(
        .x = list(unique(stats::na.omit(.data$BroodIDLaid))),
        .f = ~ {
          if (length(..1) == 0) {
            return(NA_character_)
          } else if (length(..1) == 1) {
      ...
      BroodIDLaid = dplyr::case_when(
        grepl("COHORT|PARENTAGE", .data$ExperimentID) ~ NA_character_,
