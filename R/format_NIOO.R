...
format_NIOO <- function(db = choose_directory(),
  ...
  access_tables <- c(
    "dbo_tl_AreaGroup", "dbo_tx_Area_AreaGroup", "dbo_tbl_Location",
  ...
  main_sites <- c(
    "Buunderkamp", "Westerheide", "Hoge Veluwe",
  ...
  Locations <- read.csv(paste0(table_dir, "/", "dbo_tl_AreaGroup", ".csv")) %>%
    ...
    dplyr::left_join(
      read.csv(paste0(table_dir, "/", "dbo_tx_Area_AreaGroup", ".csv")) %>%
    ...
    dplyr::left_join(
      read.csv(paste0(table_dir, "/", "dbo_tbl_Location", ".csv")) %>%
  ...
  if (is.null(species)) {
    species_filter <- species_codes$speciesEURINGCode
  } else {
    species_filter <- species_codes[species_codes$Species %in% species, ]$speciesEURINGCode
  }
  ...
  Brood_data <- create_brood_NIOO(table_dir, Locations, species_filter, pop_filter, protocol_version)
  ...
  Capture_data <- create_capture_NIOO(table_dir, Brood_data, Locations, species_filter, pop_filter, protocol_version)
  ...
  avg_mass <- Brood_data %>%
    ...
    dplyr::filter(
      .data$CaptureDate >= (.data$HatchDate_observed + 14),
  ...
  if (output_type == "csv") {
    ...
    utils::write.table(
      x = protocol_version, file = paste0(path, "\\protocol_version_NIOO.txt"),
...
#'   Used to filter the data.
#' @param protocol_version Version of the standard protocol.
#'
#' @return A data frame.

create_brood_NIOO <- function(dir, location_data, species_filter, pop_filter, protocol_version) {
  ...
  individuals <- utils::read.csv(paste0(dir, "/", "dbo_tbl_Individual", ".csv")) %>%
    ...
    dplyr::mutate(dplyr::across(
      .cols = c("RingNumber", "IndividualNumber"),
      .fns = ~ {
  ...
  Brood_data <- Brood_data %>%
    ...
    dplyr::mutate(
      Species = dplyr::case_when(
        .data$BroodSpecies == 14400 ~ species_codes[species_codes$speciesEURINGCode == 14400, ]$Species,
        .data$BroodSpecies == 14640 ~ species_codes[species_codes$speciesEURINGCode == 14640, ]$Species,
        .data$BroodSpecies == 13490 ~ species_codes[species_codes$speciesEURINGCode == 13490, ]$Species,
        .data$BroodSpecies == 14620 ~ species_codes[species_codes$speciesEURINGCode == 14620, ]$Species,
        .data$BroodSpecies == 14790 ~ species_codes[species_codes$speciesEURINGCode == 14790, ]$Species,
        .data$BroodSpecies == 15980 ~ species_codes[species_codes$speciesEURINGCode == 15980, ]$Species,
        .data$BroodSpecies == 14610 ~ species_codes[species_codes$speciesEURINGCode == 14610, ]$Species,
        .data$BroodSpecies == 11220 ~ species_codes[species_codes$speciesEURINGCode == 11220, ]$Species
      ),
      ...
      ClutchType_observed = dplyr::case_when(
        grepl(pattern = "replacement", .data$ClutchType_observed) ~ "replacement",
      ...
      LocationID = as.character(paste0(.data$PopID, "_", .data$LocationID))
    ) %>%
...
create_capture_NIOO <- function(dir, Brood_data, location_data, species_filter, pop_filter, protocol_version) {
  ...
  RawCaptures <- utils::read.csv(paste0(dir, "/", "dbo_tbl_Capture", ".csv")) %>%
    ...
    dplyr::left_join(
      utils::read.csv(paste0(dir, "/", "dbo_vw_MI_CaptureCaptureData", ".csv")) %>%
        dplyr::select(
          "CaptureID", "SpeciesID", "Observer",
          "Weight", "Tarsus",
          "P3_Length", "Age"
        ),
      by = "CaptureID"
    ...
    dplyr::mutate(
      CaptureDate = lubridate::as_date(.data$CaptureDate),
  ...
  Capture_data <- Captures_w_RingSeason %>%
    ...
    dplyr::mutate(
      Species = dplyr::case_when(
        .data$SpeciesID == 14400 ~ species_codes[species_codes$speciesEURINGCode == 14400, ]$Species,
        .data$SpeciesID == 14640 ~ species_codes[species_codes$speciesEURINGCode == 14640, ]$Species,
        .data$SpeciesID == 13490 ~ species_codes[species_codes$speciesEURINGCode == 13490, ]$Species,
        .data$SpeciesID == 14620 ~ species_codes[species_codes$speciesEURINGCode == 14620, ]$Species,
        .data$SpeciesID == 14790 ~ species_codes[species_codes$speciesEURINGCode == 14790, ]$Species,
        .data$SpeciesID == 15980 ~ species_codes[species_codes$speciesEURINGCode == 15980, ]$Species,
        .data$SpeciesID == 14610 ~ species_codes[species_codes$speciesEURINGCode == 14610, ]$Species,
        .data$SpeciesID == 11220 ~ species_codes[species_codes$speciesEURINGCode == 11220, ]$Species
      ),
    ...
    dplyr::left_join(
      location_data %>%
        dplyr::select(
          "CapturePlot" = "AreaID", "CaptureLocation" = "ID",
          "CapturePopID" = "PopID"
        ),
      by = "CaptureLocation"
    ) %>%
    dplyr::left_join(
      location_data %>%
        dplyr::select(
          "ReleasePlot" = "AreaID", "ReleaseLocation" = "ID",
          "ReleasePopID" = "PopID"
        ),
      by = "ReleaseLocation"
    ) %>%
  ...
  Capture_data <- Capture_data %>%
    ...
    dplyr::mutate(
      diff = as.integer(lubridate::ymd(.data$CaptureDate) - .data$HatchDate_observed),
      ChickAge = dplyr::case_when(
        !is.na(.data$diff) & dplyr::between(.data$diff, 0, 30) ~ .data$diff,
        TRUE ~ NA_integer_
      ),
    ...
    ) ## FIXME: Ask Marcel about individual only experiments.
...
create_individual_NIOO <- function(dir, Capture_data, location_data, species_filter, pop_filter, protocol_version) {
  ...
  individuals <- utils::read.csv(paste0(dir, "/", "dbo_tbl_Individual", ".csv")) %>%
    ...
    dplyr::mutate(
      Sex_calculated = dplyr::case_when(
        .data$Sexe %in% c(1, 3, 5) ~ "F",
        .data$Sexe %in% c(2, 4, 6) ~ "M",
        TRUE ~ NA_character_
      ),
      RingAge_category = dplyr::case_when(
        .data$RingAge %in% c(1, 2, 3) ~ "chick",
  ...
  first_captures <- utils::read.csv(paste0(dir, "/", "dbo_tbl_Capture", ".csv")) %>%
    ...
    dplyr::summarise(
      FirstCaptureLocation = dplyr::first(.data$CaptureLocation),
  ...
  Individual_data <- individuals %>%
    ...
    dplyr::mutate(
      ...
      RingSeason = dplyr::case_when(
        is.na(.data$RingYear) ~ as.integer(lubridate::year(.data$FirstCaptureDate)),
...
create_location_NIOO <- function(dir, location_data, species_filter, pop_filter, protocol_version) {
  ...
  Location_data <- utils::read.csv(paste0(dir, "/", "dbo_tbl_NestboxAppearance", ".csv")) %>%
    ...
    dplyr::filter(.data$PopID %in% pop_filter) %>%
    dplyr::select(
      "LocationID" = "Location", "NestboxID" = "ID", "LocationType" = "NestBoxType",
