...
format_UAN <- function(db = choose_directory(),
  ...
  BROOD_info <- BROOD_info %>%
    ...
    dplyr::mutate(
      ClutchSize_observed = as.integer(.data$ClutchSize_observed),
  ...
  avg_chick_data <- Capture_data %>%
    ...
    dplyr::summarise(
      AvgChickMass_capture = mean(.data$Mass, na.rm = TRUE),
  ...
  Brood_data <- Brood_data %>%
    ...
    dplyr::rowwise() %>%
    dplyr::mutate(
      AvgChickMass = ifelse(!is.na(.data$AvgChickMass_capture), .data$AvgChickMass_capture, .data$AvgChickMass),
  ...
  if (output_type == "csv") {
    ...
    utils::write.table(
      x = protocol_version, file = paste0(path, "/protocol_version_UAN.txt"),
...
create_brood_UAN <- function(BROOD_info, CAPTURE_info, species_filter, pop_filter, protocol_version) {
  ...
  Tarsus_method <- CAPTURE_info %>%
    dplyr::group_by(.data$BroodID) %>%
    dplyr::summarise(
      TarsusAlt = length(stats::na.omit(.data$TarsusAlt)) > 0,
      TarsusStd = length(stats::na.omit(.data$TarsusStandard)) > 0,
      .groups = "drop"
    ) %>%
    dplyr::mutate(OriginalTarsusMethod = dplyr::case_when(
      .data$TarsusAlt == "TRUE" ~ "Alternative",
  ...
  Brood_data <- BROOD_info %>%
    ...
    dplyr::mutate(
      ...
      Species = dplyr::case_when(
        .data$Species == "pm" ~ species_codes[species_codes$speciesEURINGCode == 14640, ]$Species,
        .data$Species == "pc" ~ species_codes[species_codes$speciesEURINGCode == 14620, ]$Species
      ),
      ClutchType_observed = dplyr::case_when(
        .data$ClutchType_observed %in% c(1, 9) ~ "first",
        .data$ClutchType_observed %in% c(2, 6, 7) ~ "second",
        .data$ClutchType_observed %in% c(3, 4, 5, 8) ~ "replacement"
      ),
      ClutchSizeError = dplyr::case_when(
        .data$ClutchSizeError == "J" ~ 0,
        .data$ClutchSizeError == "N" ~ 2
      ),
      ClutchSize_min = dplyr::case_when(
        (.data$ClutchSize_observed - .data$ClutchSizeError) < 0 ~ 0L,
    ...
    dplyr::mutate(dplyr::across(
      c("FemaleID", "MaleID"),
      ~ dplyr::case_when(
        stringr::str_detect(., "^[a-zA-Z]+$") ~ NA_character_,
        TRUE ~ .
      )
    )) %>%
...
create_capture_UAN <- function(CAPTURE_info, species_filter, pop_filter) {
  ...
  Capture_data <- CAPTURE_info %>%
    ...
    dplyr::mutate(IndvID = dplyr::case_when(
      stringr::str_detect(.data$IndvID, "^[a-zA-Z]+$") ~ NA_character_,
      TRUE ~ .data$IndvID
    )) %>%
    ...
    dplyr::mutate(
      ...
      CaptureTime = dplyr::na_if(paste(.data$CaptureTime %/% 1,
        stringr::str_pad(
          string = round((.data$CaptureTime %% 1) * 60),
          width = 2,
          pad = "0"
        ),
        sep = ":"
    ...
    dplyr::mutate(Sex_observed = dplyr::case_when(
      .data$Sex %in% c("M", "M?") ~ "M",
      .data$Sex %in% c("W", "W?") ~ "F",
      TRUE ~ NA_character_
    )) %>%
    ...
    dplyr::mutate(
      Age_observed_new = dplyr::case_when(
        (is.na(.data$Age_observed) | .data$Age_observed == 0) & .data$CaptureMethod %in% c("P", "PP") ~ 1L,
      ...
      ChickAge = dplyr::case_when(
        .data$Age_observed > 5 ~ .data$Age_observed,
        TRUE ~ NA_integer_
      )
    ) %>%
    ...
    calc_age(
      ID = .data$IndvID, Age = .data$ischick,
...
create_individual_UAN <- function(INDV_info, Capture_data, species_filter, protocol_version) {
  ...
  individuals <- Capture_data %>%
    ...
    dplyr::summarise(
      Species = dplyr::case_when(
        length(unique(.data$Species)) == 2 ~ "CCCCCC",
      ...
      BroodIDLaid = dplyr::case_when(
        RingAge == "chick" ~ dplyr::first(.data$BroodID),
  ...
  Location_data <- BOX_info %>%
    dplyr::mutate(
      ...
      NestboxID = dplyr::case_when(
        .data$LocationType == "NB" ~ .data$GBPL,
        TRUE ~ NA_character_
      ),
      Latitude = .data$Y_deg,
