...
format_WYT <- function(db = choose_directory(),
  ...
  Individual_data <- create_individual_WYT(Capture_data, protocol_version)

  # LOCATION DATA

  message("Compiling location information...")

  Location_data <- create_location_WYT(Brood_data, Capture_data, protocol_version)
...
create_brood_WYT <- function(db, species_filter){
  ...
  Brood_data <- Brood_data_raw %>%
    ...
    dplyr::mutate(BreedingSeason = .data$year,
                  ...
                  Species = dplyr::case_when(.data$species == "b" ~ species_codes[species_codes$speciesEURINGCode == 14620, ]$Species,
                                             .data$species == "g" ~ species_codes[species_codes$speciesEURINGCode == 14640, ]$Species,
                                             .data$species == "c" ~ species_codes[species_codes$speciesEURINGCode == 14610, ]$Species,
                                             .data$species == "n" ~ species_codes[species_codes$speciesEURINGCode == 14790, ]$Species,
                                             .data$species == "m" ~ species_codes[species_codes$speciesEURINGCode == 14400, ]$Species)) %>%
...
create_capture_WYT <- function(db, Brood_data, species_filter){
  ...
  Chick_captures_old <- Chick_captures_old %>%
    dplyr::mutate(Species = dplyr::case_when(toupper(.data$species_code) == "GRETI" ~ species_codes[species_codes$speciesEURINGCode == 14640, ]$Species,
                                             toupper(.data$species_code) == "BLUTI" ~ species_codes[species_codes$speciesEURINGCode == 14620, ]$Species,
  ...
  Chick_captures_new <- Chick_captures_new %>%
    dplyr::mutate(Species = dplyr::case_when(toupper(.data$bto_species_code) == "GRETI" ~ species_codes[species_codes$speciesEURINGCode == 14640, ]$Species,
                                             toupper(.data$bto_species_code) == "BLUTI" ~ species_codes[species_codes$speciesEURINGCode == 14620, ]$Species,
