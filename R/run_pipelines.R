...
#'
#' @param path File path. Location of the root directory of all data hosted at SPI-Birds. Note, the sub-directories for each pipeline must include the unique code of the pipeline as seen in `study_codes`.
#' @param studyID The unique (three-letter, one-digit code) code of studies to format as listed in `study_codes`.
#' @param speciesID The six-letter code of species to include as listed in `species_codes`. Note, this argument takes precedence over argument studyID (i.e., if a study doesn't have the requested species it will not be formatted.)
...
#' HAR_data <- run_pipelines(studyID = "HAR-1", speciesID = "PARMAJ", output_type = "R")
#'
#' }
...
run_pipelines <- function(path = choose_directory(),
  ...
  # If study is NULL use all study
  if(is.null(studyID)){

    studyID <- study_codes$studyID
  ...
  # Drop studies from Access-based primary data if running on Mac
  Access_studies <- c("HOG-1", "OOS-1", "VLI-1", "BUU-1", "LIE-1", "WAR-1", "WES-1", "AMM-1")

  if(grepl(pattern = 'mac', x = OS)){

    if(length(studyID[which(studyID %in% Access_studies)] > 0)){
      warning(paste0('Pipelines not run for the following sites due to OS incompatibility: ',
                    toString(studyID[which(studyID %in% Access_studies)]),
                    ". To obtain standard format data for these studies, please run on a Windows OS.")
              )
    }

    studyID <- studyID[which(!(studyID %in% Access_studies))]
  ...
  # Firstly, check if there are any cases where a requested study does not have any info on a given species
  missing_species <- study_species_combos %>%
    dplyr::filter(.data$studyID %in% {{studyID}}) %>%
  ...
  if(any(missing_species$total_species == 0)){
    ...
    missing_species %>%
      ...
      purrr::pwalk(.l = list(.data$studyID),
                   .f = ~{

                     message(paste0('Study ', ..1, ' has no information on (one of) the focal species and has been excluded.'))

                   })

  }

  # Now just work with those studies where the study and species are present
  study_species_subset <- study_species_combos %>%
    dplyr::filter(.data$studyID %in% {{studyID}} & .data$speciesID %in% {{speciesID}}) %>%
    dplyr::left_join(dplyr::select(study_codes, studyID, pipelineID), by = "studyID") %>%
    dplyr::group_by(.data$pipelineID) %>%
    dplyr::summarise(Species = list(c(unique(speciesID))),
                     Studies = list(c(unique(studyID))))

  # Find the file path for each of the pipelines of interest
  all_dirs <- list.dirs(path = path, full.names = TRUE, recursive = FALSE)
  all_dirs <- all_dirs[grepl(pattern = paste(study_species_subset$pipelineID, collapse = "|"), all_dirs)]
  all_dirs <- gsub(pattern = "\\", replacement = "/", x = all_dirs, fixed = TRUE)

  # For each pipeline, run the studies-species combinations requested
  # Return R lists rather than generating .csv files
  R_objects <- purrr::pmap(.l = list(dirs = all_dirs,
                                     pipeline = study_species_subset$pipelineID,
                                     studies = study_species_subset$Studies,
                                     species = study_species_subset$Species),
                           .f = function(dirs, pipeline, studies, species){
  ...
  if(output_type == "R"){
    ...
    if("Measurement_data" %in% names(R_objects) & "Experiment_data" %in% names(R_objects)) { # v2.0
