...
#'
#' \strong{LayDate_min/LayDate_max & HatchDate_min/HatchDate_max}: Accuracy of laying and hatch date
...
#' We generate temporary IndvIDs using BroodID + letter designation
#' to enable CaptureID generation for the Capture table.
#'
...
#' 74).
#'
#' \item #9.	CaptureAlive and ReleaseAlive are for now a guess.
#' The original data has codes in Finnish that are translated as:
#' Assume for now that "D" means in Condition means Dead
#' Assume all other birds (even NA ones) are released alive
#' TODO: Check with data owner
#' Assume for now that "P", "M", "E" means that they are released alive
#' Assume "T" means killed when handling
#'
#' Condition code translations according to a translator.
#' | Code | Finnish/English      | Meaning         |
#' |------|---------------------|-----------------|
#' | D    | Död/Dead            | Dead            |
#' | V    | Voimissaan/Vigorous | Healthy/Strong  |
#' | H    | Heikko              | Weak            |
#' | NA   |                     | Not recorded    |
#'
#' ExtraInfo code translations:
#' | Code | Finnish/English | Meaning           |
#' |------|-----------------|-------------------|
#' | P    | Päästetty        | Released          |
#' | T    | Tuntematon       | Unknown           |
#' | NA   |                  | Not recorded      |
#'
#' BirdStatus code translations:
#' | Code | Finnish      | English (meaning)   |
#' |------|--------------|---------------------|
#' | M    | Merkitty     | Marked/tagged       |
#' | P    | Päästetty    | Released            |
#' | E    | Elossa       | Alive               |
#' | T    | Tapettu      | Killed              |
#' | O    | Otettu       | Taken/removed       |
#' | H    | Hukkunut     | Lost/drowned        |
#' | NA   |              | Not recorded        |
#'
#' We cannot determine the full identity of the captured individual without a
#' matching record in Capture_data because we only know the last 2 digits of the
#' ring number. We generate temporary IndvIDs using BroodID + the last 2 digits
#' to enable CaptureID generation. These can still be used to
#' estimate AvgChickMass, AvgTarsus for a given BroodID; however,
#' these may represent data entry errors. Check with data custodian if needed.}
#'
...
format_HAR <- function(db = choose_directory(),
  ...
  if (output_type == "csv") {
    ...
    utils::write.csv(x = Capture_data, file = paste0(path, "\\Capture_data_HAR.csv"), row.names = FALSE)

    utils::write.csv(x = Location_data, file = paste0(path, "\\Location_data_HAR.csv"), row.names = FALSE)

    utils::write.table(
      x = protocol_version, file = paste0(path, "\\protocol_version_HAR.txt"),
...
create_brood_HAR <- function(db, species_filter) {
  ...
  # Many of these are subsequently removed,
  # but it makes it easier for non-Finnish speakers to
  # see what is being removed.
  Brood_data <- extract_paradox_db(
    path = db,
    file_name = "HAR_PrimaryData_Pesat.DB"
  ) %>%
    dplyr::rename(
      "BreedingSeason" = "Vuos", "LocationID" = "Nuro",
      "BroodID" = "Anro", "Species" = "Laji",
      "ClutchType_observed" = "Pesa", "FemaleID" = "Naaras",
      "MaleID" = "Koiras", "LayingDate_day" = "Mpv",
      "LayingDate_month" = "Mkk", "LayingDateError" = "Mtar",
      "HatchDate_day" = "Kpv", "HatchDate_month" = "Kkk",
      "HatchDateError" = "Ktar",
      "Incubation" = "Halku", "ClutchSize_observed" = "Mulu",
      "BroodSize" = "Kuor",
      "NumberFledged_observed" = "Lent", "ReasonFailed" = "Tsyy",
    ...
    dplyr::mutate(Species = dplyr::case_when(
      .data$Species == "FICHYP" ~
        species_codes$Species[which(species_codes$speciesEURINGCode == 13490)],
      .data$Species == "PARCAE" ~
        species_codes$Species[which(species_codes$speciesEURINGCode == 14620)],
      .data$Species == "PARMAJ" ~
        species_codes$Species[which(species_codes$speciesEURINGCode == 14640)],
      .data$Species == "PARATE" ~
        species_codes$Species[which(species_codes$speciesEURINGCode == 14610)]
    )) %>%
    dplyr::filter(
      !is.na(.data$Species) & .data$Species %in% species_filter
    ) %>%
    # Add pop and plot id
    dplyr::mutate(
      PopID = "HAR",
    ...
    dplyr::mutate(ClutchType_observed = dplyr::case_when(
      .data$ClutchType_observed == 1 ~ "first",
    ...
    dplyr::mutate(
      LayDate_observed = as.Date(
        paste(.data$LayingDate_day,
          .data$LayingDate_month, .data$BreedingSeason,
          sep = "/"
        ),
        format = "%d/%m/%Y"
      ),
      HatchDate_observed = as.Date(
        paste(.data$HatchDate_day,
          .data$HatchDate_month, .data$BreedingSeason,
          sep = "/"
        ),
        format = "%d/%m/%Y"
      )
    ...
    dplyr::arrange(
      .data$BreedingSeason,
      .data$Species, .data$FemaleID, .data$LayDate_observed
    ) %>%
    # Calculate clutchtype
    dplyr::mutate(
      ClutchType_calculated =
        calc_clutchtype(data = ., na.rm = FALSE, protocol_version = "1.1")
    ) %>%
    dplyr::mutate(
      LayDate_min = dplyr::case_when(
        .data$LayingDateError == "1" ~ as.Date(.data$LayingDate_day - 1),
        .data$LayingDateError == "2" ~ as.Date(.data$LayingDate_day - 2),
        .data$LayingDateError == "3" ~ as.Date(.data$LayingDate_day - 3),
        .data$LayingDateError == "4" ~ as.Date(.data$LayingDate_day - 7)
      ),
      LayDate_max = dplyr::case_when(
        .data$LayingDateError == "1" ~ as.Date(.data$LayingDate_day + 1),
        .data$LayingDateError == "2" ~ as.Date(.data$LayingDate_day + 2),
        .data$LayingDateError == "3" ~ as.Date(.data$LayingDate_day + 3),
        .data$LayingDateError == "4" ~ as.Date(.data$LayingDate_day + 7)
      ),
      HatchDate_min = dplyr::case_when(
        .data$HatchDateError == "1" ~ as.Date(.data$HatchDate_day - 1),
        .data$HatchDateError == "2" ~ as.Date(.data$HatchDate_day - 2),
        .data$HatchDateError == "3" ~ as.Date(.data$HatchDate_day - 3),
        .data$HatchDateError == "4" ~ as.Date(.data$HatchDate_day - 7)
      ),
      HatchDate_max = dplyr::case_when(
        .data$HatchDateError == "1" ~ as.Date(.data$HatchDate_day + 1),
        .data$HatchDateError == "2" ~ as.Date(.data$HatchDate_day + 2),
        .data$HatchDateError == "3" ~ as.Date(.data$HatchDate_day + 3),
        .data$HatchDateError == "4" ~ as.Date(.data$HatchDate_day + 7)
      ...
      ### TODO: When updating to v2.0.0,
      # check with data custodian what the codes 0-9 mean
      ExperimentID = dplyr::case_when(
        !is.na(.data$ExperimentID) ~ "OTHER",
        TRUE ~ NA_character_
      )
    ) %>%
    # Arrange columns correctly
    dplyr::select(
      "BroodID", "PopID", "BreedingSeason", "Species", "Plot",
  ...
  Brood_data <- Brood_data %>%
    dplyr::mutate(dplyr::across(
      c("FemaleID", "MaleID"),
      ~ dplyr::case_when(
        stringr::str_detect(., "^[:alpha:]{1,2}[-][:digit:]{5,6}$") ~ .,
        TRUE ~ NA_character_
      )
    ))
...
create_nestling_HAR <- function(db, Brood_data) {
  ...
  Nestling_data <- extract_paradox_db(path = db, file_name = "HAR_PrimaryData_Pullit.DB") %>%
    dplyr::rename(
      "BreedingSeason" = "Vuos", "LocationID" = "Nuro", "BroodID" = "Anro",
  ...
  Nestling_data <- Nestling_data %>%
    ...
    dplyr::mutate(ChickAge = as.integer(.data$CaptureDate - .data$HatchDate_observed))
...
create_capture_HAR <- function(db, Brood_data, species_filter, return_errors) {
  ...
  Capture_data <- extract_paradox_db(path = db, file_name = "HAR_PrimaryData_Rengas.DB") %>%
    dplyr::rename(
      "RingSeries" = "Sarja", "RingNumber" = "Mista",
  ...
  Capture_data <- Capture_data %>%
    ...
    dplyr::mutate(Species = dplyr::case_when(
      .data$Species == "FICHYP" ~ species_codes$Species[which(species_codes$speciesEURINGCode == 13490)],
    ...
    dplyr::mutate(
      ...
      # Assume "T" means killed when handling
      #
      # Condition code translations according to translator.
      # | Code | Finnish/English      | Meaning         |
      # |------|---------------------|-----------------|
      # | D    | Död/Dead            | Dead            |
      # | V    | Voimissaan/Vigorous | Healthy/Strong  |
      ...
      CaptureAlive = dplyr::case_when(
        .data$BirdStatus == "D" ~ FALSE,
        is.na(.data$BirdStatus) ~ FALSE,
        TRUE ~ TRUE
      ),
      ReleaseAlive = dplyr::case_when(
        .data$BirdStatus %in% c("P", "M", "E") ~ TRUE,
        .data$ExtraInfo == "P" ~ TRUE,
        is.na(.data$BirdStatus) & is.na(.data$ExtraInfo) ~ FALSE,
        TRUE ~ FALSE
      )
    )
  ...
  Adult_capture <- Capture_data %>%
    ...
    dplyr::select(
      "IndvID", "BreedingSeason", "CaptureDate", "CaptureTime",
      "ObserverID", "LocationID", "CaptureAlive", "ReleaseAlive",
      "BroodID", "Species", "Sex", "Age", "WingLength", "Mass",
      "CaptureType", "BirdStatus", "Last2DigitsRingNr", "ChickAge"
    )
  ...
  indv_chick_capture_only <- indv_chick_capture %>%
    ...
    # Join in Brood_data (with HatchDate_observed) so we can determine ChickAge
    dplyr::mutate(
      Capture_type = "Chick",
      Last2DigitsRingNr = NA,
      ChickAge = as.integer(.data$CaptureDate - .data$HatchDate_observed),
      IndvID = paste(.data$RingSeries, .data$RingNumber, sep = "-")
    ) %>%
    dplyr::select(
      "IndvID", "BreedingSeason", "CaptureDate", "CaptureTime",
  ...
  non_matching_records <- indv_chick_capture %>%
    dplyr::mutate(
      Last2DigitsRingNr = stringr::str_sub(.data$RingNumber, start = -2),
      IndvID = paste(.data$RingSeries, .data$RingNumber, sep = "-")
    ) %>%
    dplyr::semi_join(
      Nestling_data %>%
  ...
  unique_individual_captures <- indv_chick_capture %>%
    ...
    dplyr::left_join(Brood_data %>%
      dplyr::select("BroodID", "HatchDate_observed"), by = "BroodID") %>%
    dplyr::mutate(ChickAge = as.integer(.data$CaptureDate - .data$HatchDate_observed)) %>%
    dplyr::select(
      "IndvID", "BreedingSeason", "CaptureDate", "CaptureTime",
      "ObserverID", "LocationID", "BroodID", "Species", "Sex", "Age",
  ...
  indv_chick_record_conflict <- indv_chick_capture %>%
    ...
    dplyr::left_join(
      Nestling_data %>%
        dplyr::select("BroodID", "Last2DigitsRingNr",
          ...
          "WingLengthNestling" = "WingLength", "HatchDate_observed", "ChickAge"
        ),
      by = c("BroodID", "Last2DigitsRingNr")
    ) %>%
    ...
    dplyr::mutate(
      Mass = dplyr::case_when(
        is.na(.data$MassNestling) ~ .data$Mass,
        TRUE ~ .data$MassNestling
      ),
      WingLength = dplyr::case_when(
        is.na(.data$WingLengthNestling) ~ .data$WingLength,
  ...
  expanded_multi_chick_capture <- flat_multi_chick_capture %>%
    ...
    dplyr::mutate(RingNumber = purrr::map2(
      .x = .data$RingNumber,
    ...
    dplyr::select(
      "LocationID", "BroodID", "RingSeries", "RingNumber",
  ...
  expanded_multi_chick_capture <- expanded_multi_chick_capture %>%
    dplyr::mutate(dplyr::across(
      .cols = tidyselect::where(~ !is.null(names(.))),
  ...
  multirecord_captures <- expanded_multi_chick_capture %>%
    ...
    dplyr::mutate(ChickAge = dplyr::case_when(
      !is.na(.data$ChickAge) ~ as.integer(.data$ChickAge),
      TRUE ~ as.numeric(.data$CaptureDate - .data$HatchDate_observed)
    )) %>%
    dplyr::mutate(IndvID = paste(.data$RingSeries, .data$RingNumber, sep = "-")) %>%
    dplyr::select(
      "IndvID", "BreedingSeason", "CaptureDate", "CaptureTime",
  ...
  # 6a. Filter unringed individuals. These are given temporary IDs based on BroodID + letter designation
  # to enable CaptureID generation while maintaining link to brood for analysis.
  unringed_chicks <- nocapture_nestlings %>%
    dplyr::filter(toupper(.data$Last2DigitsRingNr) %in% LETTERS) %>%
    dplyr::mutate(
      IndvID = paste0(.data$BroodID, "_", toupper(.data$Last2DigitsRingNr)),
      ObserverID = NA_character_,
      Sex = NA_character_,
      Age = "PP",
  ...
  ringed_chicks_nocapture <- nocapture_nestlings %>%
    dplyr::filter(!toupper(.data$Last2DigitsRingNr) %in% LETTERS) %>%
    dplyr::mutate(
      # Generate temporary ID for chicks with no complete ring record
      # Use BroodID + last 2 digits
      # These should be verified with data custodian as they may represent data entry errors
      IndvID = paste0(.data$BroodID, "_", .data$Last2DigitsRingNr),
      ObserverID = NA_character_,
      Sex = NA_character_,
      Age = "PP",
      CaptureType = NA_character_
  ...
  message(paste0("There are ", nrow(ringed_chicks_nocapture), " nestling records where we cannot translate the last 2 digits into an IndvID"))
  ...
  Capture_data_expanded <- dplyr::bind_rows(
    Adult_capture, indv_chick_capture_only, indv_chick_multirecord,
  ...
  Capture_data_output <- Capture_data_expanded %>%
    # Set non-conforming ring-based IDs to NA, but keep temporary IDs (BroodID_*)
    dplyr::mutate(IndvID = dplyr::case_when(
      stringr::str_detect(.data$IndvID, "^[:alpha:]{1,2}[-][:digit:]{5,6}$") ~ .data$IndvID,
      stringr::str_detect(.data$IndvID, "^[:digit:]{4}_[:digit:]+_[:digit:]+_") ~ .data$IndvID, # Temporary IDs
      TRUE ~ NA_character_
    )) %>%
    ...
    ### TODO: Check with data owner
    dplyr::mutate(ChickAge = dplyr::case_when(
      dplyr::between(.data$ChickAge, 0, 99) ~ as.integer(.data$ChickAge),
      TRUE ~ NA_integer_
    )) %>%
    ...
    # Generate CaptureID (required in v1.1.0+): IndvID + capture number for that individual
    ...
    dplyr::select(
      "CaptureID",
      "IndvID", "Species", "BreedingSeason", "CaptureDate", "CaptureTime",
      ...
      "Sex_observed" = "Sex", "BroodID", "CaptureType", "BirdStatus",
...
create_individual_HAR <- function(Capture_data, protocol_version) {
  ...
  Indv_data <- Capture_data %>%
    ...
    dplyr::summarise(
      ...
      RingSeason = dplyr::first(as.integer(.data$BreedingSeason)),
    ...
    dplyr::mutate(
      BroodIDLaid = ifelse(.data$RingAge == "chick", .data$BroodIDLaid, NA_character_),
      BroodIDFledged = .data$BroodIDLaid
    ) %>%
...
create_location_HAR <- function(db, protocol_version) {
  ...
  Location_data <- Location_full %>%
    ...
    dplyr::summarise(
      NestboxID = unique(.data$LocationID),
      PopID = "HAR",
