...
#### Experiment "BSM - Griffioen et al. 2019 PeerJ" was done with hatched nestlings,
# not with eggs.
...
create_individual_PEW <- function(Capture_data){

  Individual_data <-
    Capture_data %>%
    # #### Remove rows without date >> ALREADY REMOVED IN CAPTURE DATA
...
Capture_data <- create_capture_PEW(data = pew_data)
...
create_capture_PEW <- function(data) {
  ...
  Capture_data <-
    ...
    dplyr::mutate(#### ASSUMPTION: There is no captureID info, so far I made up this
                  ...
                  CaptureTime = ifelse(is.na(ObservationTimeH),
                                       ...
                                       paste(sprintf("%02d", round(ObservationTimeH, 0)),
                                             "00", sep = ":")),
                  Sex_observed = ifelse(Sex == "Chick", NA, substr(Sex, 1, 1)),
  ...
  #### Get hatching & ringing dates from parents
  cluch_dates <-
    Capture_data %>%
    dplyr::filter(Method == "Catch adults nestbox") %>%
    dplyr::select(BreedingSeason, IndvID, Sex, Age, PartnerId, LocationID, CaptureDate,
                  Method, ClutchSize, DateEgg1, HatchDateD0, DateRingingChicks, BroodID) %>%
    group_by(BroodID) %>%
    mutate(DateEgg1 =  purrr::map_chr(.x = list(unique(na.omit(as.character(DateEgg1)))), .f = ~{
                                                if(length(..1) == 0){
                                                  return(NA_character_)
                                                } else if(length(..1) == 1){
                                                  return(..1)
            ...
            DateEgg1 = as.Date(DateEgg1),
            HatchDateD0 =  purrr::map_chr(.x = list(unique(na.omit(as.character(HatchDateD0)))), .f = ~{
                                                    if(length(..1) == 0){
                                                      return(NA_character_)
                                                    } else if(length(..1) == 1){
                                                      return(..1)
            ...
            HatchDateD0 = as.Date(HatchDateD0),
            DateRingingChicks =  purrr::map_chr(.x = list(unique(na.omit(as.character(DateRingingChicks)))), .f = ~{
                                                          if(length(..1) == 0){
                                                            return(NA_character_)
                                                          } else if(length(..1) == 1){
                                                            return(..1)
           ...
           #### ???
           # LaidDate = dplyr::case_when(!is.na(DateEgg1) ~ DateEgg1,
           #                             is.na(HatchDateD0) ~  HatchDateD0 - 15)) %>%

    group_by(BroodID) %>%
    dplyr::summarise(DateEgg1Clutch = unique(DateEgg1),
                     HatchDateClutch = unique(HatchDateD0),
                     DateRingingChicksClutch = unique(DateRingingChicks)) %>%
    ungroup()


  Capture_data <-
    Capture_data %>%
    left_join(cluch_dates, by = "BroodID") %>%
    #### The age of captured chicks in days since hatching
    dplyr::mutate(ChickAge = if_else(Age_observed == 1,
                                     as.integer(difftime(CaptureDate, HatchDateClutch)),
                                     NA_integer_) %>%
                  # LaidDate =
                  # BroodIDLaid = ) %>%
    #### Exclude observations without the Date, as they are not "captures"
...
#Remove unneeded Capture columns (AFTER USING THER IN OTHER FUNCTIONS, INDIVIDUAL DATA)
Capture_data <- Capture_data %>%
  dplyr::select(-BroodIDLaid, -BroodIDFledged, -HatchDate)
...
# No Capture date at chickringing
# sum(is.na(pew_data$Date[pew_data$Method == "ChickRinging"]))
# 501



#### VARIOUS ASSUMPTIONS:
# For CaptureDate: - remove individuals without Date
...
#### --------------------------------------~
#### BROOD DATA                         #### done, some errors, need check
#### --------------------------------------~
...
create_brood_PEW <- function(data) {
  ...
  parents_brood_data <-
    ...
    # May use this way after fixing problematic couples (see below)
    ...
    dplyr::mutate(FemaleID = case_when(Sex == "Female" ~ IndvID,
                  ...
                  ExperimentID = dplyr::case_when(Experiment == "BSM - Griffioen et al. 2019 PeerJ" ~
                                                  ...
                                                  Experiment == "2h Temp D4 + Handicaping Male: Griffioen et al. 2019 Front Ecol&Evol" ~
                                                    "SURVIVAL; PARENTAGE")) %>%
    #### Remove unnecesary variables which cause duplicated rows
    ...
    parents_brood_data_2 <-
      ...
      dplyr::mutate(Plot = NA_character_,
                    ...
                    # NumberFledged_observed = NumberOfRingedChicks,
                    NumberFledged = NumberOfRingedChicks,
                    NumberFledged_min = NA_integer_,
                    NumberFledged_max = NA_integer_,
                    AvgEggMass = NA,
                    NumberEggs = NA_integer_,
                    #### Metadata states that only first clutches are recorded.
                    ClutchType_observed = "first")
                    #### ERRORS WHEN USING THIS FUNCTION, ignore for now
                    # ClutchType_calculated = calc_clutchtype(data = ., na.rm = FALSE))

    #### CHECK: For some reason seems to works when calculating apart as vector ....
    parents_brood_data_3 <-
      parents_brood_data_2 %>%
      bind_cols(ClutchType_calculated = calc_clutchtype(data = ., na.rm = FALSE))


    #### Get measurements per brood
    chicks_measures <-
      pew_data %>%
    ...
    Brood_data <-
      parents_brood_data_3 %>%
      left_join(chicks_measures, by = "BroodID") %>%
      #### CHECK: Rename temporarily, remove after solving issues
      #    with the calc_clutchtype function
      dplyr::rename(NumberFledged_observed = NumberFledged,
