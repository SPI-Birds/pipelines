...
create_brood_AMM   <- function(connection) {
  ...
  Brood_data <- dplyr::tbl(connection, "Broods") %>%
    ...
    dplyr::mutate(BroodID = as.character(.data$BroodID),
                  ...
                  Species = dplyr::case_when(.data$Species == 1L ~ !!species_codes$speciesID[species_codes$speciesCode == 10001],
                                             .data$Species == 2L ~ !!species_codes$speciesID[species_codes$speciesCode == 10002],
                                             .data$Species == 3L ~ !!species_codes$speciesID[species_codes$speciesCode == 10005],
                                             .data$Species == 4L ~ !!species_codes$speciesID[species_codes$speciesCode == 10004]),
                  AvgEggMass = NA_real_,
...
create_capture_AMM <- function(Brood_data, connection) {
  ...
  Adult_capture <- Catches_table %>%
    ...
    dplyr::collect() %>%
    dplyr::mutate(Species = dplyr::case_when(.data$CatchSpecies == 1L ~ !!species_codes$speciesID[species_codes$speciesCode == 10001],
                                             .data$CatchSpecies == 2L ~ !!species_codes$speciesID[species_codes$speciesCode == 10002],
                                             .data$CatchSpecies == 3L ~ !!species_codes$speciesID[species_codes$speciesCode == 10005],
                                             .data$CatchSpecies == 4L ~ !!species_codes$speciesID[species_codes$speciesCode == 10004]),
  ...
  Chick_capture <- Chick_catch_tables %>%
    ...
    dplyr::mutate(HatchDay = dplyr::case_when((.data$HatchDay >= 500 | .data$HatchDay < 0) ~ NA_integer_,
                                              TRUE ~ .data$HatchDay),
                  # All chicks are GT
                  Species = species_codes$speciesID[species_codes$speciesCode == 10001],
