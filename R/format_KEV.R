...
format_KEV <- function(db = choose_directory(),
  ...
  if(output_type == "csv"){
    ...
    utils::write.table(x = protocol_version, file = paste0(path, "\\protocol_version_KEV.txt"),
...
create_brood_KEV <- function(db, species_filter){
  ...
  Brood_data <- Brood_data %>%
    ...
    dplyr::mutate(Species = dplyr::case_when(.data$Species == "FICHYP" ~ species_codes$Species[which(species_codes$speciesEURINGCode == 13490)],
                                             ...
                                             .data$Species == "PARCIN" ~ species_codes$Species[which(species_codes$speciesEURINGCode == 14480)],
                                             .data$Species == "PHOPHO" ~ species_codes$Species[which(species_codes$speciesEURINGCode == 11220)])) %>%
