...
#'
#'\strong{Species}: Primarily PARMAJ and some FICHYP, but also a few CYACAE, PERATE, POEPAL, and SITEUR
#'
#'\strong{IndvID}:
#'
#' \strong{LocationID}:
...
format_VOM <- function(db = choose_directory(),
  ...
  nest_data <- readxl::read_xlsx(path = paste0(db, "/VOM_PrimaryData.xlsx"), guess = 5000, col_types = "text") %>%
    ...
    dplyr::mutate(BreedingSeason = as.integer(.data$Yea),
...
format_VOM <- function(db = choose_directory(),
  ...
  #### INDIVIDUAL DATA
  message("Compiling individual information...")
  Individual_data_temp <- create_individual_VOM(nest_data)
...
>>>>>>> 9d7e9875d9099f5b2ec01634c92c846c527b7728
