...
#db <- '/Users/chloenater/Dropbox/PiedFlycatcher_IPM/Raw_Data/PrimaryData_for_Masterpipeline'
...
create_capture_PFN <- function(Nest_data, IPMR_data, ReRingTable, badIDs){
  ...
  Capture_data <- Capture_data %>%
    dplyr::mutate(
      ...
      ## 3.1) Captures dates, times & observers
...
## Function to extract capture data from nest files
...
## Function to extract capture data from IPMR files
...
## NOTE: This is still the version from the original EDM pipeline.
##       Will have to be updated/adjusted once (location) data for other
##       populations is submitted to SPI-Birds.
