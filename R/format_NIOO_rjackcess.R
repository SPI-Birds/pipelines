...
format_NIOO_rjackcess <- function(db = choose_directory(),
                        species = NULL,
                        pop = NULL,
                        path = ".",
                        output_type = "R"){
...
create_brood_NIOO <- function(dir, location_data, species_filter, pop_filter){
  ...
  Brood_data <- utils::read.csv(paste0(dir, "/", "dbo_tbl_Brood", ".csv"), na.strings = c("", "NA")) %>%
    ...
    dplyr::left_join(location_data %>%
                     ...
                     by = "BroodLocationID") %>%
    dplyr::mutate(Species = dplyr::case_when(.data$BroodSpecies == 14400 ~ species_codes[species_codes$speciesEURINGCode == 14400, ]$Species,
...
create_capture_NIOO <- function(dir, Brood_data, location_data, species_filter, pop_filter){
  ...
  Capture_data <- Captures_w_RingSeason %>%
    ...
    dplyr::ungroup() %>%
    dplyr::mutate(Species = dplyr::case_when(.data$SpeciesID == 14400 ~ species_codes[species_codes$speciesEURINGCode == 14400, ]$Species,
...
create_location_NIOO <- function(dir, location_data, species_filter, pop_filter, protocol_version){
  ...
  Location_data <- utils::read.csv(paste0(dir, "/", "dbo_tbl_NestboxAppearance", ".csv")) %>%
    ...
    # Drop locations that are not a nest box or mist net location
    dplyr::filter(!is.na(.data$LocationType)) %>%
