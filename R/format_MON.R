...
format_MON <- function(db = choose_directory(),
  ...
  Capture_data <- create_capture_MON(
    db = db,
  ...
  Individual_data <- create_individual_MON(
    Capture_data,
    Brood_data,
    protocol_version,
    verbose
  ...
  Location_data <- create_location_MON(
    db = db,
    Capture_data,
    Brood_data,
    protocol_version
  ...
  if (output_type == "csv") {
    ...
    utils::write.table(
      x = protocol_version, file = paste0(path, "/protocol_version_MON.txt"),
...
create_capture_MON <- function(db, species_filter, pop_filter) {
  Full_capture_data <- utils::read.csv(paste0(db, "/MON_PrimaryData_MORPH.csv"), na.strings = "") %>%
    ...
    dplyr::mutate(Species = dplyr::case_when(
      .data$espece == "ble" ~ species_codes$Species[species_codes$speciesEURINGCode == 14620],
      .data$espece == "noi" ~ species_codes$Species[species_codes$speciesEURINGCode == 14610],
      .data$espece == "cha" ~ species_codes$Species[species_codes$speciesEURINGCode == 14640],
      ...
      .data$espece == "sit" ~ species_codes$Species[species_codes$speciesEURINGCode == 14790],
      .data$espece == "moi" ~ "UN-IDENTIFIED SPARROW",
      .data$espece == "moid" ~ "HOUSE SPARROW",
      .data$espece == "moif" ~ species_codes$Species[species_codes$speciesEURINGCode == 15980],
      .data$espece == "non" ~ species_codes$Species[species_codes$speciesEURINGCode == 14400]
    )) %>%
    # Filter by species
    dplyr::filter(.data$Species %in% species_filter) %>%
    dplyr::mutate(
      CaptureDate = suppressWarnings(as.Date(.data$date_mesure, format = "%d/%m/%Y")),
      CaptureTime = .data$heure,
      BreedingSeason = as.numeric(.data$an),
      IndvID = purrr::pmap_chr(
        .l = list(.data$bague),
        .f = ~ {
      ...
      ActionTaken = dplyr::case_when(
        .data$action == "bcj" ~ "Ringed",
    ...
    dplyr::mutate(
      ...
      ExperimentDescription1 = dplyr::case_when(
        .data$exp_ad == "FMR" ~ "Metabolic measurement",
  ...
  Full_capture_data <- Full_capture_data %>%
    ...
    dplyr::mutate(ReleasePlot = dplyr::case_when(
      is.na(.data$Destination) ~ .data$lieu,
    ...
    dplyr::mutate(
      ...
      Tarsus = dplyr::case_when(
        is.na(.data$tarsed) & is.na(.data$tarseg) ~ NA_real_,
  ...
  Chick_capture_data <- readr::read_delim(paste0(db, "/MON_PrimaryData_POUS.csv"), show_col_types = FALSE) %>%
    ...
    dplyr::mutate(
      CaptureDate = suppressWarnings(as.Date(.data$date_mesure, format = "%d/%m/%Y")),
      CaptureTime = stringr::str_sub(.data$heure, 1, 5),
      BreedingSeason = .data$an,
      IndvID = dplyr::case_when(
        grepl(pattern = "no_ident", x = .data$bague) ~ NA_character_,
        TRUE ~ .data$bague
      ),
      Mass = .data$poids,
      ObserverID = .data$obs,
      ...
      Age_observed = 1,
      ExperimentDescription1 = dplyr::case_when(
        .data$expou == "1" ~ "Manipulation affect selective value (e.g. cross foster)",
        .data$expou == "2" ~ "No manipulation that affects selective value (e.g. weighing)"
      ),
      ExperimentDescription2 = dplyr::case_when(
        .data$exp_p == "alimente" ~ "Supplemental feeding",
    ...
    dplyr::mutate(
      ReleasePlot = purrr::map2_chr(
        .x = .data$DestBoxNumber,
        .y = .data$CapturePlot,
        .f = ~ {
          if (length(.x) == 2) {
            return(.x[1])
    ...
    dplyr::filter(.data$CapturePopID %in% pop_filter) %>%
    dplyr::select(
      "IndvID", "Species", "BreedingSeason", "CaptureDate", "CaptureTime", "FoundDead", "ObserverID",
  ...
  Capture_data <- Capture_data %>%
    ...
    dplyr::mutate(IndvID = dplyr::case_when(
      nchar(.data$IndvID) %in% c(6, 7, 8) & stringr::str_detect(.data$IndvID, "^[:digit:]+$") ~ .data$IndvID,
      TRUE ~ NA_character_
    )) %>%
...
create_brood_MON <- function(db, species_filter, pop_filter) {
  Brood_data <- utils::read.csv(paste0(db, "/MON_PrimaryData_DEMO.csv"), na.strings = "") %>%
    ...
    dplyr::mutate(
      Species = dplyr::case_when(
        .data$espece == "ble" ~ species_codes$Species[which(species_codes$speciesEURINGCode == 14620)],
        .data$espece == "noi" ~ species_codes$Species[which(species_codes$speciesEURINGCode == 14610)],
        .data$espece == "cha" ~ species_codes$Species[which(species_codes$speciesEURINGCode == 14640)],
        .data$espece == "eto" ~ "STARLING",
        .data$espece == "grp" ~ "UN-IDENTIFIED CREEPER",
        .data$espece == "grpj" ~ "SHORT-TOED TREECREEPER",
        .data$espece == "grpd" ~ "EURASIAN TREECREEPER",
        .data$espece == "hup" ~ "CRESTED TIT",
        .data$espece == "sit" ~ species_codes$Species[which(species_codes$speciesEURINGCode == 14790)],
        .data$espece == "moi" ~ "UN-IDENTIFIED SPARROW",
        .data$espece == "moid" ~ "HOUSE SPARROW",
        .data$espece == "moif" ~ species_codes$Species[which(species_codes$speciesEURINGCode == 15980)],
        .data$espece == "non" ~ species_codes$Species[which(species_codes$speciesEURINGCode == 14400)]
      ),
      Plot = .data$lieu,
      BoxNumber = .data$nic,
      ...
      BroodID = dplyr::case_when(
        !is.na(.data$LayingDate) ~ paste(.data$BreedingSeason, .data$Plot,
      ...
      MaleID = .data$mbag,
      FemaleID = .data$fbag,
      ParasiteTreatment = dplyr::case_when(
        .data$extnt == "1" ~ "Protected",
        .data$extnt == "2" ~ "Unprotected"
      ),
      Brood_ExperimentDescription1 = dplyr::case_when(
        .data$expou == "1" ~ "No intervention that will affect breeding success",
      ...
      FailureCause = dplyr::case_when(
        .data$mort == "ABA" ~ "Abandoned",
    ...
    dplyr::mutate(ExperimentID = dplyr::case_when(
      (!is.na(.data$Crossfostering_treatment) | !is.na(.data$Brood_ExperimentDescription2) | .data$ParasiteTreatment == "Treated" | .data$expou == "2") ~ "OTHER",
  ...
  Brood_data <- Brood_data %>%
    ...
    dplyr::mutate(
      FemaleID = dplyr::case_when(
        nchar(.data$FemaleID) %in% c(6, 7, 8) & stringr::str_detect(.data$FemaleID, "^[:digit:]+$") ~ .data$FemaleID,
        TRUE ~ NA_character_
      ),
      MaleID = dplyr::case_when(
        nchar(.data$MaleID) %in% c(6, 7, 8) & stringr::str_detect(.data$MaleID, "^[:digit:]+$") ~ .data$MaleID,
        TRUE ~ NA_character_
      )
    )
  ...
  Brood_data <- Brood_data %>%
    dplyr::mutate(BroodID = dplyr::case_when(
      .data$BroodID %in% {
        duplicated_broods
      } & is.na(.data$FemaleID) & is.na(.data$MaleID) ~ NA_character_,
      TRUE ~ .data$BroodID
    )) %>%
...
create_individual_MON <- function(Capture_data, Brood_data, protocol_version, verbose) {
    ...
    apply(1, function(x, Brood_data) {
      ...
      if (!is.na(x["BroodIDFledged"])) {
        ...
        possible_nest <- Brood_data %>%
          dplyr::filter(
            .data$BreedingSeason == split_info[1],
        ...
        possible_nest <- Brood_data %>%
          dplyr::filter(
            .data$BreedingSeason == lubridate::year(CaptureDate),
  ...
  Individual_data <- Capture_data %>%
    ...
    dplyr::summarise(
      Species = purrr::map_chr(
        .x = list(stats::na.omit(unique(.data$Species))),
        .f = ~ {
          if (length(..1) == 1) {
            return(..1)
          } else {
      ...
      RingAge = dplyr::case_when(
        is.na(dplyr::first(Age_observed)) ~ NA_character_,
        dplyr::first(Age_observed) <= 3 ~ "chick",
        dplyr::first(Age_observed) > 3 ~ "adult"
      ),
      Sex = purrr::pmap_chr(
        .l = list(
          list(unique(.data$ObservedSex)),
  ...
  Individual_data_single_records <- dplyr::left_join(Individual_data, BroodAssignment, by = "IndvID") %>%
    ...
    dplyr::mutate(BroodIDFledged = dplyr::case_when(
      is.na(.data$BroodIDFledged) ~ .data$BroodIDLaid,
...
create_location_MON <- function(db, Capture_data, Brood_data, protocol_version) {
  ...
  nestbox_latlong_outside <- utils::read.csv(paste0(db, "/MON_PrimaryData_OffSiteLocation.csv"), na.strings = "") %>%
    ...
    dplyr::mutate(
      LocationID_join = paste(.data$st, .data$ni_localisation, sep = "_"),
      latitude = .data$la,
      longitude = .data$lo
    ) %>%
    dplyr::select("LocationID_join", "latitude", "longitude") %>%
  ...
  non_latlong_locations <- dplyr::bind_rows(inside_locations, nest_locations) %>%
    ...
    dplyr::summarise(
      NestboxID = dplyr::case_when(
        grepl(unique(.data$LocationID),
          pattern = "NB"
        ) ~ unique(.data$LocationID),
        TRUE ~ NA_character_
      ),
      ...
      Habitat = dplyr::case_when(
        .data$Plot %in% c("ava", "fel", "mur", "rou") ~ "deciduous",
        .data$Plot %in% c(
          "fil", "ari", "gra",
          "pir", "tua"
        ) ~ "evergreen",
        .data$Plot %in% c(
          "bot", "cef", "fac", "font",
          "gram", "mas", "mos", "val", "zoo"
        ) ~ NA_character_, # TODO: set to urban when updating with later protocol version
        TRUE ~ NA_character_
      )
    ) %>%
    dplyr::select(-"Plot")
...
identify_PopID_MON <- function(variable) {
  dplyr::case_when(
    variable %in% c("ava", "fel", "mur", "fil", "ari", "gra") ~ "MUR",
    ...
    variable == "ven" ~ "MTV",
    variable %in% c(
      "hs", "aul", "mes", "aig", "bon", "cap", "crt", "gen", "mal",
      "mau", "mrt", "olm", "pac", "pie", "pog", "pon",
      "pre", "pue", "sfl", "stb", "tcb", "tcv", "vic", "vol"
    ) ~ "MIS",
...
find_box <- function(string, position = 1) {
  ...
  if (is.na(suppressWarnings(as.numeric(split_string[position])))) {
    ...
    if (position == 1) {
      ...
      return(c(
        paste(split_string[1:(position - 1)], collapse = ""),
