...
format_SAG <- function(db = choose_directory(),
  ...
  db <- "/Users/tyson/Documents/academia/institutions/NIOO/SPI-Birds/my_pipelines/SAG/data/SAG_Sagunto_Spain/"
  ...
  brood_data <- dplyr::tbl(connection, "Datos de Nidificaci\u00f3n") %>%
    ...
    dplyr::transmute(PopID = "SAG",
                     ...
                     Plot= toupper(.data$Caja),
                     LocationID = toupper(.data$Localizacion),
                     ...
                     ExperimentID = .data$Experimentos,
    ) %>%
  ...
  names(dplyr::tbl(connection, "DATOS PRIMER ANILLAMIENTO") %>%
          tibble::as_tibble())
  ...
  rr_data_ring <- dplyr::tbl(connection, "DATOS PRIMER ANILLAMIENTO") %>%
    ...
    dplyr::transmute(PopID = "SAG",
                     Species = species_codes[species_codes$SpeciesID == 14640,]$Species,
                     IndvID = dplyr::case_when(stringr::str_detect(.data$Anilla,  "^[:alnum:]{6,8}$") ~ .data$Anilla,
                                               TRUE ~ NA_character_),
                     Plot= toupper(.data$CodigoCaja),
                     LocationID = toupper(.data$Localizacion),
                     BroodID = .data$CodigoNido,
                     ...
                     CaptureDate = .data$FechaCaptura,
                     BreedingSeason = format(CaptureDate, "%Y"),
