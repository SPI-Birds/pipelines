...
create_brood_STR <- function(db,
  ...
  broods <-  readr::read_delim(paste0(db, "/STR_PrimaryData_Brood.csv"), col_types = my_cols(.default = 'c',
    ...
    dplyr::mutate(plotID = tolower(.data$site),
                  ...
                  # Describe reported experiments (for experiment table), but details are required from data custodians
                  experimentID = dplyr::case_when(.data$remarques == "manip IMMUNO" ~ "br_1",
                                                  .data$remarques %in% c("mâle mort pdt la capture", "porte tombée", "nichoir tombé", "nichoir disparu") ~ "br_2",
                                                  .data$experience %in% c("SURVIVAL", "SURVIVAL ") ~ "br_3",
                                                  .data$experience %in% c("OTHER") ~ "br_4",
                                                  .data$prelev_nid %in% c("œuf", "poussin", "œuf;poussin", "poussin;femelle") ~ "br_5"),
                  experimentType = dplyr::case_when(experimentID %in% c("br_1") ~ "treatment",
                                                    experimentID %in% c("br_2") ~ "non_experimental",
                                                    experimentID %in% c("br_3", "br_4") ~ "To_be_determined",
                                                    experimentID %in% c("br_5") ~ "sampling"),
                  treatmentDetails = dplyr::case_when(experimentID == "br_1" ~ "immunology treatment",
                                                      experimentID == "br_2" ~ "accidental event resulting in failing breeding event",
                                                      experimentID == "br_3" ~ "experience related to survival",
                                                      experimentID == "br_4" ~ "no information",
                                                      experimentID == "br_5" ~ "sampling on egg, chick or female, no information about the sampling",
                                                      TRUE ~ NA_character_),
  ...
  nestbox_loc <- readr::read_delim(paste0(db, "/STR_PrimaryData_Location.csv"), show_col_types = FALSE) %>%
    ...
    dplyr::mutate(locationDetails = dplyr::case_when(stringr::str_detect(.data$remarques, "bois jusqu'") & count == 1 ~ "Wooden nesting box",
                  ...
                  endYear = dplyr::case_when(count == 1 & stringr::str_detect(.data$remarques, "non suivi 2020|non suivi 2020, 2021|non suivi 2020,2021|bois jusqu'en 2020") ~ 2019,
                                             ...
                                             count == 1 & stringr::str_detect(.data$remarques, "bois jusqu'en 2018") ~ 2017,
                  ...
                  startYear = dplyr::case_when(count == 2 & stringr::str_detect(.data$remarques, "non suivi 2019,2020,2021|non suivi 2020, 2021|non suivi 2021") ~ 2022,
                                               ...
                                               count == 2 & stringr::str_detect(.data$remarques, "bois jusqu'en 2020") ~ 2020,
                                               count == 2 & stringr::str_detect(.data$remarques, "bois jusqu'en 2018") ~ 2018,
  ...
  mn_location <- Capture_data %>%
    ...
    dplyr::mutate(decimalLatitude = NA_real_, #see data custodian for further details
                  ...
                  locationDetails = dplyr::case_when(.data$type_capture == "FILET VERTICAL" ~ "Mist net",
                                                     ...
                                                     TRUE ~ NA_character_),
                  elevation = NA_real_,
                  habitatID = dplyr::case_when(.data$captureSiteID == "ROB" ~ "J2", #Check with data custodians for more details
                                               .data$captureSiteID == "STR" ~ "J1",
                                               TRUE ~ "G1"),
...
create_experiment_STR <- function(Brood_data) {
  ...
  # TODO: Details needed from data custodian
